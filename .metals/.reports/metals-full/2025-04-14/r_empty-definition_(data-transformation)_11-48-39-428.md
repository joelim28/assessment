...
      df_master.show()  // Display the DataFrame to ensure the header is read correctly

      // Convert DataFrame to RDD
      val rdd_master = df_master.rdd.map(row => row.mkString(","))
...
      if (idx == 0) iter.drop(1) else ite@@r
    }.map { row =>
