...
object ParquetReader {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val df_master = spark.read.parquet(parquetFilePath_master)
      df_master.show()
      
      // Read and display master data
      //val rdd_master = readParquetFile(spark, parquetFilePath_master)
      val rdd_master = df_master.rdd
      
      val transformedRDD = rdd@@.map(row => (row.getAsString, row.getAsInt * 2, row.getAsString))
      transformedRDD.collect().foreach(println)

      val header = rdd_master.first()
      //showTopRecords(rdd_master, 10)  // Display top 10 records with header
