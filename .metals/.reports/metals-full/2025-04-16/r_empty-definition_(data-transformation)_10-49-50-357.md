error id: org/apache/spark/sql/SparkSession#createDataFrame(+2).
file:///C:/Users/josephlim/Projects/data-transformation/src/main/scala/Main.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol org/apache/spark/sql/SparkSession#createDataFrame(+2).
  ...
  object ParquetReader {
    def main(args: Array[String]): Unit = {
      ...
      try {
        ...
        writeParquetFile(spark, rdd_report, parquetFilePath_output)
...
def writeParquetFile(spark: SparkSession, rdd: RDD[String], filePath: String): Unit = {
  ...
  try {
    ...
    val dataRDD = rdd.filter(_ != Columnheader)

    // Convert RDD[String] to DataFrame without specifying schema

    val rowRDD = dataRDD.map(_.split(",")).map(arr => Row(arr: _*))
 

    // Create DataFrame with inferred schema
    val df = spark.createDataFrame@@(rowRDD, Columnheader)
    df.show()
