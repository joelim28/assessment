...
non-local guesses:
	 ...
	 -updatedColumnNames#
	 -updatedColumnNames().
	 -scala/Predef.updatedColumnNames.
	 -scala/Predef.updatedColumnNames#
	 -scala/Predef.updatedColumnNames().
offset: 995
...
object ParquetReader {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val (rdd_master_stg1, masterColumnNames) = readParquetFile(spark, parquetFilePath_master)
      showTopRecords(rdd_master_stg1, masterColumnNames, 11)  // Display top 11 records with header
    
      val (rdd_master_stg2, deduped_rdd_master_ColumnNames) = dedupeRDD(spark, rdd_master, masterColumnNames, Seq("geographical_location_oid"))
      showTopRecords(deduped_rdd_master, updatedCo@@lumnNames, 11)  // Display top 11 records with header
    
      /*val filtered_rdd_master = filterRecords(deduped_rdd_master, masterColumnNames, "geographical_location_oid", "Y")
